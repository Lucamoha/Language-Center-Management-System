...
public class CoursesPanel extends JPanel {
    private static final Logger log = LoggerFactory.getLogger(CoursesPanel.class);
    ...
    private void onEdit() {
        ...
        Course selected = model.getRow(row);
    ...
    private void onDelete() {
        int row = table.getSelectedRow();
        if (row < 0) {
            MessageBox.warn(this, "Vui lòng chọn một khóa học để xóa.");
            return;
        }
        Course selected = model.getRow(row);

        if (!MessageBox.confirm(this, "Bạn có chắc muốn xóa khóa học: " + selected.getCourseName() + "?"))
            return;

        new SwingWorker<Void, Void>() {
            @Override
            protected Void doInBackground() {
                service.delete(selected.getCourseID());
                return null;
            }
            ...
            protected void done() {
                try {
                    get();
                    MessageBox.info(CoursesPanel.this, "Đã xóa khóa học.");
                    loadData(null);
                } catch (Exception ex) {
                    handleException(ex);
                }
